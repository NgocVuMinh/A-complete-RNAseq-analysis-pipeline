...
while IFS= read -r sample; do
    ...
    samtools sort -@ 3 "./processed/${sample}.bam" -o "./processed/${sample}_sorted.bam" 2>&1 | tee -a $log_file_path
    echo "$(date) - Sorting completed for ${sample}" 2>&1 | tee -a $log_file_path
    printf '\n' 2>&1 | tee -a "$log_file_path"

    # Indexing BAM
    echo "$(date) - Indexing BAM file: ${sample}" 2>&1 | tee -a $log_file_path
    samtools index -@ 3 "./processed/${sample}_sorted.bam" 2>&1 | tee -a $log_file_path 2>&1 | tee -a $log_file_path
